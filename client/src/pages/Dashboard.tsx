    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-background p-6 text-center">
        ...
        <p className="text-muted-foreground max-w-md mb-8">
          We encountered an error while trying to load your projects. Please try
        ...
        <ScrollArea className="flex-1 px-3 py-4">
          <div className="space-y-1">
            ...
            {filteredProjects.map((project) => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full mr-2 ${selectedProjectId === project.id ? "bg-primary" : "bg-muted-foreground/30"}`}
                />
                {project.name}
              </Button>
            ))}
...
function ProjectCard({
  ...
  const pendingFeaturesList =
    project?.features?.filter(
      (i: any) => i.status === "pending" || i.status === "open",
    ) || [];
  const pendingImprovementsList =
    project?.improvements?.filter(
      (i: any) => i.status === "pending" || i.status === "open",
    ) || [];
  const pendingBugsList =
    project?.bugs?.filter(
      (i: any) => i.status === "pending" || i.status === "open",
    ) || [];
  const [hoveredType, setHoveredType] = useState<
    "features" | "improvements" | "bugs" | null
  >(null);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out ${hoveredType ? "max-h-40 opacity-100 mt-2" : "max-h-0 opacity-0"}`}
        >
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-1">
                {pendingFeaturesList.slice(0, 3).map((item: any) => (
                  <li
                    key={item.id}
                    className="text-[11px] text-foreground border-l-2 border-blue-500 pl-2 line-clamp-1"
                  >
                    {item.description}
                  </li>
                ))}
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  {pendingImprovementsList.slice(0, 3).map((item: any) => (
                    <li
                      key={item.id}
                      className="text-[11px] text-foreground border-l-2 border-purple-500 pl-2 line-clamp-1"
                    >
                      {item.description}
                    </li>
                  ))}
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-1">
                {pendingBugsList.slice(0, 3).map((item: any) => (
                  <li
                    key={item.id}
                    className="text-[11px] text-foreground border-l-2 border-red-500 pl-2 line-clamp-1"
                  >
                    {item.description}
                  </li>
                ))}
...
function ProjectDetails({ id, onBack }: { id: number; onBack: () => void }) {
  ...
  const handleUpdate = async (formData: any) => {
    ...
    console.log("Updating project with data:", finalData);

    await updateProject.mutate({ id, ...finalData });
    setIsEditOpen(false);
  };
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <button
            onClick={onBack}
            className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 w-fit transition-colors"
          >
      ...
      <AnimatePresence mode="wait">
        ...
        >
              ...
              <TabsContent value="technical" className="mt-0 space-y-6">
                    ...
                    <div className="prose prose-sm prose-invert max-w-none text-muted-foreground min-h-[100px]">
                      ...
                      >
                        {project.frontendDetails ||
                          "No frontend details documented."}
                      </ReactMarkdown>
                    </div>
                  </div>
                  <div className="bg-card/30 border border-border/50 rounded-xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-card/30 border border-border/50 rounded-xl p-6 space-y-4">
                    ...
                    <div className="prose prose-sm prose-invert max-w-none text-muted-foreground min-h-[100px]">
                      ...
                      >
                        {project.testUserDetails ||
                          "No test accounts documented."}
                      </ReactMarkdown>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-card/30 border border-border/50 rounded-xl p-6 space-y-4">
                  ...
                  <div className="prose prose-sm prose-invert max-w-none text-muted-foreground">
                    ...
                    >
                      {project.authDetails ||
                        "No authentication details documented."}
                    </ReactMarkdown>
                  </div>
                </div>
              </TabsContent>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="space-y-4">
              <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70 px-1">
                Recent Activity
              </h4>
  ...
  return (
    <div className="bg-card border border-border/50 rounded-xl overflow-hidden">
      ...
      <div className="p-4 bg-black/20 relative min-h-[100px]">
        {content ? (
          <pre
            className={`text-xs font-mono text-muted-foreground whitespace-pre-wrap ${!isRevealed ? "blur-sm select-none" : ""}`}
          >
            {content}
          </pre>
        ) : (
